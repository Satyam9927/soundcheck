...
let () =
  if Assurance.profile.id <> "kong-traditional-http-v8"
     || Assurance.profile.version <> 8
...
Unsupported:
  ...
  (match report.Soundcheck_core.Report.assurance with
   | Some assurance
     when assurance.profile = "kong-traditional-http-v8"
  ...
  if not (contains human "Assurance: kong-traditional-http-v8 (conservative)")
